...
class TfidfPreprocess:
    ...
    def __get_distinct_words_labels(self, dataset=False):
        distinct_words = {}
        labels = []
        count = 0
        if dataset:
    ...
    def __calc_tfidf(self, dataset, q_list, a_list):
        qids = {}
        aids = {}
        with gzip.open(dataset, 'rb') as f:
            next(f)
    ...
    def __cosine_sim(self, dataset, q_scores, a_scores, temp):
        sim_vec = []
        line_count = 0
        with gzip.open(dataset, 'rb') as f:
            next(f)
    ...
    def preprocess_tfidf(self, dataset):
        first_pass = self.__get_distinct_words_labels(dataset)
        distinct_word = first_pass[0]
        labels = first_pass[1]
        tfidf = self.__calc_tfidf(dataset, {}, {})
        q_scores = tfidf[0]
        a_scores = tfidf[1]
        sims = np.asarray(self.__cosine_sim(dataset, q_scores, a_scores, distinct_word))
